#!/bin/bash
# A bash script to push to GitHub (which automatically deploys static GitHub Pages)
...
echo -e "${GREEN}Start Pushing to GitHub... Please wait for the pushed message.${NC}"
git add .
git commit -m "$commitMessage"
git push origin main

echo -e "${GREEN}Successfully pushed to GitHub!${NC}"
echo -e "${BLUE}Since this is a static site repository ($REPO_NAME), pushing to 'main' automatically triggers the deployment.${NC}"
echo -e "${BLUE}Your changes will be live on GitHub Pages in 1-2 minutes.${NC}"
